...
if st.button("Get Data"):
    ...
    div_table = pd.DataFrame({
        "Total_Dividends_Per_Year": div_yearly
    })

    div_table["Dividend_YoY_Growth_%"] = (
        div_table["Total_Dividends_Per_Year"].pct_change() * 100
    )
    ...
    st.success(f"Downloaded {len(df)} rows successfully.")

# ==========================================================
# DISPLAY SECTION (ONLY IF DATA EXISTS)
# ==========================================================
...
if "stock_table" in st.session_state and "div_table" in st.session_state:
    ...
    stock_yoy = pd.to_numeric(
        stock_table["YoY_Growth_%"], errors="coerce"
    ).dropna()
    ...
    div_yoy = pd.to_numeric(
        div_table["Dividend_YoY_Growth_%"], errors="coerce"
    ).dropna()
    ...
    stock_growth = st.number_input(
        "Expected Annual Stock Growth (0.05 = 5%)",
        value=0.07,
        step=0.01,
        format="%.4f",
        key="stock_growth"
    ...
    div_growth = st.number_input(
        "Expected Annual Dividend Growth (0.03 = 3%)",
        value=0.03,
        step=0.01,
        format="%.4f",
        key="div_growth"
    ...
    if st.button("Run Portfolio Projection"):
        ...
        last_dividend_per_share = (
            div_table["Total_Dividends_Per_Year"].iloc[-1]
            if not div_table.empty else 0
        )
        ...
        total_gain_pct = (
            estimated_portfolio_worth / total_cash_contributed - 1
        ) * 100
